...
class GANSeg(object) :
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % 50 == 0:
                ...
                save_name = self.dataset+str(step) + '_params_latest.pt'
                self.save(os.path.join(self.result_dir, self.dataset, 'model'), savename=save_name)
    ...
    def test(self):
        ...
        #加载模型参数
        model_list = glob(os.path.join(self.result_dir, self.dataset,'model', '*41850*'))#或者*best
        print(model_list)
        if not len(model_list) == 0:
            ...
            print(" [*] Load FAILURE")
            return
        #这里我理解是是否使用生成对抗网络,否则即文献中说的仅使用Unet分割,no_gan,no_seg设置为false时，训练所需的显存大大增加,将channal设置为原来的1/2,bach_size设置为4后所需显存约18G
        if not self.no_seg:
            self.seg.eval()
            #输出图片的地址,如没有则创建一个
            ...
            if not os.path.isdir(out_dir):
                os.makedirs(out_dir)
            #加载trainB的图片还是testA,初始默认值为0,即加载testA的图片
            #对teatA/trainB中的图片进行预测
            do_train_B = 0
            ...
            for n, (real_A, yt) in enumerate(loader):
                ...
                for n, (real_B, yt) in enumerate(self.testB_loader):
                    ...
                    with torch.no_grad():
                        ...
                        out_np = out.clone()
                        out = out.argmax(dim=1)
                        print('testB', n, out.shape, torch.min(real_B), torch.max(real_B))
        ...
        if not self.no_gan:
            ...
            for n, (real_B, yt) in enumerate(self.testB_loader):
                ...
                fake_B2B, _, fake_B2B_heatmap = self.genA2B(real_B)
                #============================调试代码======================================
                AA=tensor2numpy(fake_B2B_heatmap[0])
                BB=tensor2numpy(fake_B2A_heatmap[0])
                CC=tensor2numpy(fake_B2A2B_heatmap[0])
                print((tensor2numpy(denorm(real_B[0]))).shape)
                print(cam(AA, self.img_size).shape)
                print(tensor2numpy(denorm(fake_B2B[0])).shape)
                print(cam(BB, self.img_size).shape)
                print(tensor2numpy(denorm(fake_B2A[0])).shape)
                print(cam(CC, self.img_size).shape)
                print(tensor2numpy(denorm(fake_B2A2B[0])).shape)
                # (256, 256, 1)
                # (256, 256, 3)
                # (256, 256, 1)
                # (256, 256, 3)
                ...
                #                       tensor2numpy(denorm(fake_B2A2B[0]))), axis=0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B_%d.png' % (n + 1)), tensor2numpy(denorm(real_B[0])) * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'camB_%d.png' % (n + 1)),cam(tensor2numpy(denorm(real_B[0])),self.img_size)* 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2B%d.png' % (n + 1)), tensor2numpy(denorm(fake_B2B[0])) * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'camB2B%d.png' % (n + 1)), cam(tensor2numpy(denorm(fake_B2B[0])),self.img_size) * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2A_%d.png' % (n + 1)), tensor2numpy(denorm(fake_B2A[0])) * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'camB2A_%d.png' % (n + 1)), cam(tensor2numpy(denorm(fake_B2A[0])),self.img_size) * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2A2B_%d.png' % (n + 1)), tensor2numpy(denorm(fake_B2A2B[0])) * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'camB2A2B_%d.png' % (n + 1)),cam(tensor2numpy(denorm(fake_B2A2B[0])),self.img_size) * 255.0)
        return